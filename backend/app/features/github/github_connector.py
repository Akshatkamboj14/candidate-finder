...
# small helper: safely request and handle rate limit headers
    def _req(
        self, path: str, params: dict = None, raw: bool = False
    ) -> Optional[requests.Response]:
        url = path if path.startswith("http") else f"{self.GITHUB_API_BASE}{path}"
    ...
    def _get_user_search(self, query: str, per_page: int = 30, page: int = 1):
        # uses GitHub Search Users API: /search/users?q={query}
        q = quote_plus(query)
        path = "/search/users"
        params = {"q": query, "per_page": per_page, "page": page}
        return self._req(path, params=params)

    def _get_user(self, username: str):
        return self._req(f"/users/{username}")

    def _get_user_repos(self, username: str, per_page: int = 5):
        # list user repos (sorted by updated)
        params = {"per_page": per_page, "sort": "updated", "direction": "desc"}
        return self._req(f"/users/{username}/repos", params=params)

    def _get_repo_readme(self, owner: str, repo: str):
    ...
    def fetch_and_index_github_users(
        self, query: str, max_users: int = 50, per_user_repos: int = 3
    ) -> List[Dict]:
        """
        ...
        while users_indexed < max_users:
            resp = self._get_user_search(query, per_page=per_page, page=page)
            if resp is None:
                break
            data = resp.json()
            ...
            for u in items:
                ...
                # fetch user details
                user_resp = self._get_user(username)
                if not user_resp:
                    results_summary.append(
                        {"username": username, "indexed": False, "reason": "user_not_found"}
                    )
                    continue
                user_obj = user_resp.json()
                # fetch top repos
                repos_resp = self._get_user_repos(username, per_page=per_user_repos)
                ...
                for r in top_repos:
                    ...
                    try:
                        rd = self._get_repo_readme(owner, repo_name)
                        if rd:
                            readmes[repo_name] = rd
                    except Exception:
                        pass
                # normalize to profile document
                profile_text = self.normalize_user_to_profile(user_obj, top_repos, readmes)
                # embedding
                try:
                ...
                try:
                    ...
                    # Extract evidence using structured extractor
                    evidence_map = {}
                    try:
                    ...
                    results_summary.append({"username": username, "indexed": False, "reason": f"upsert_err:{e}"})
