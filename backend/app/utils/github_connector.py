...
import json
import requests
from backend.app.utils.skills import extract_evidence_for_skills_from_text
...
def _req(
    path: str, params: dict = None, raw: bool = False
...
def fetch_and_index_github_users(
    query: str, max_users: int = 50, per_user_repos: int = 3
) -> List[Dict]:
    """
    ...
    while users_indexed < max_users:
        ...
        for u in items:
            ...
            try:
                ...
                evidence_map = {}
                try:
                    # use extract_evidence_for_skills_from_text or extract_evidence_from_text as available
                    if "extract_evidence_for_skills_from_text" in globals():
                        # returns dict: { skill: [snippets...] }
                        evidence_map = extract_evidence_for_skills_from_text(profile_text)
                    elif "extract_evidence_from_text" in globals():
                        # older extractor may return list -> convert to dict
